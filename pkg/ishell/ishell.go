...
func (s *Shell) executeKubectl(cmd []string) (error) {
	if len(cmd) == 0 {
		return nil
	}

	// Point to the specific kubeconfig
	if s.kubeConfig == "" {
		return errors.New("No kubeconfig provided")
	}
	cmd = append(cmd, "--kubeconfig="+s.kubeConfig)

	// Allow for "kubectl" to be passed at the beginning of the command
	if cmd[0] == "kubectl" || cmd[0] == "k" {
		cmd = cmd[1:]
	}
...
// Customization for safekubectl: sets the kubeconfig location
func (s *Shell) KubeConfig(kubeConfig string) {
